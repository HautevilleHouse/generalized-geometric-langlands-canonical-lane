#!/usr/bin/env python3
"""Canonical-lane closure guard for the generalized geometric langlands super-repo."""
...
PRIMARY_KEYS = ('kappa_correspondence', 'sigma_hecke', 'kappa_compact', 'rho_rigidity', 'derived_transfer')
...
def compute_report(data,sigma_star,strict):
    ...
    gates={f'GGL_G{i}':('PASS' if p else 'FAIL') for i,p in enumerate(gate_pass, start=1)}
    gates['GGL_G6']='PASS' if coh_pass else 'FAIL'
    gates['GGL_GM']='PASS' if gm_pass else 'FAIL'
    blockers={name:([] if status=='PASS' else ['missing/nonpositive or not theorem-level']) for name,status in gates.items() if name not in ('GGL_G6','GGL_GM')}
